...
void read(const char *infile, const char *savefile = "~/FLOW5/PLOT/NoneFlowЕ30_40.root")
{
...
const char *STR;
    ...
    for (int k = 0; k < Nb; k++)
    {
        ...
        for (int m = 0; m < NN; m++)
        {
            ...
            RMSvep[k][m] = pow(sigmaX(HDiffVobs[k][m], 1), 0.5) / HDiffRES[k][m]->GetMean()-0.5*HDiffVobs[k][m]->GetMean()*pow(sigmaX(HDiffRES[k][m], 1), 0.5)/pow(HDiffRES[k][m]->GetMean(),1.5) ;
...
cout << "Ok "<<endl;
    for (int kk = 0; kk < Nb; kk++)
    {
        ...
        IntMc[kk] = vMC[kk], ErMc[1] = pow(sigmaX(HMC[kk], 1), 0.5);
        IntV2[kk] = v2[kk], ErV2[kk] = pow(sigmaX(SV2[kk], 0.25 / sv2[kk]), 0.5);
        IntV4[kk] = v4[kk];
    ...
    grvEp[kk]->SetLineWidth(1);
    //grvEp[kk]->Draw("SAME P");
    grvEp[kk]->SetTitle("V_{2}{Event Plane}");

    //Рефренсный поток с погрешностями
    double x[1] = {0.5}, y[1] = {vMC[kk]}, ex[1] = {0.}, ey[1] = {pow(sigmaX(HMC[kk], 1), 0.5)};
    ...
    double x2[1] = {1.5}, y2[1] = {v2[kk]}, ex2[1] = {0.}, ey2[1] = {pow(sigmaX(SV2[kk], 0.25 / sv2[kk]), 0.5)};
    grIntv2[kk] = new TGraphErrors(1, x2, y2, ex2, ey2);
    grIntv2[kk]->SetName("v2");
    grIntv2[kk]->SetMarkerStyle(20);

    double x4[1] = {2.5}, y4[1] = {v4[kk]}, ex4[1] = {0.}, ey4[1] = {0};
    ...
    double x5[1] = {3.5}, y5[1] = {vEP[kk]}, ex5[1] = {0.}, ey5[1] = {pow(sigmaX(HVobs[kk], 1), 0.5) / pow(HRES[kk]->GetMean(), 0.5)};
...
TCanvas *c1 = new TCanvas("c1", "demo bin labels1", 10, 10, 600, 600);
...
TCanvas *c2 = new TCanvas("c2", "demo bin labels", 10, 10, 600, 600);
    ...
    TH1F *h = new TH1F("h", "Reference flow V_{2}", 4, 0., 4.);
    const Int_t nx = 4;
    ...
    double ymin = min(v4[cent] - ey4[0], v2[cent] - ey4[0]);
    ...
    TCanvas *c3 = new TCanvas("c3", "demo bin labels3", 10, 10, 600, 600);
    ...
    TGraphErrors *grIvE = new TGraphErrors(Nb, binCent, IntEv, RMSbinCent, ErEv);
    grIv4->SetMarkerStyle(21);
    grIv4->SetMarkerSize(1);
    grIv4->SetMarkerColorAlpha(kRed, 1);
    grIv4->SetLineColorAlpha(kRed, 1);
    grIv4->SetLineWidth(2);
